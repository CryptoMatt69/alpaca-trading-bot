    ...
    with lock:
        try:
            ...
            if side in ["close_long", "close_short"]:
                if current_side and (
                    (side == "close_long" and current_side == "long") or
                    (side == "close_short" and current_side == "short")
                ):
            ...
            # ✅ FIXED PRICE FETCH (ONLY CHANGE)
            bar = api.get_latest_bar(symbol)
            current_price = float(bar.c) if bar and bar.c else 0.0
