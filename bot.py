...
# ----------------------------
# NEW HELPER FOR ALL-DAY ENTRY
def submit_smart_entry(symbol, qty, side, current_price):
    clock = api.get_clock()
    ...
    with lock:
        try:
            ...
            if side=="long":
                ...
                order = submit_smart_entry(symbol, qty, "buy", current_price)

                # TP: 3 shares
                tp_price = round(current_price * (1 + TP_PERCENT), 2)
                api.submit_order(symbol=symbol, qty=3, side="sell", type="limit", limit_price=tp_price, time_in_force="day")

                # SL: all shares
                sl_price = round(current_price * (1 - SL_PERCENT), 2)
                api.submit_order(symbol=symbol, qty=qty, side="sell", type="stop", stop_price=sl_price, time_in_force="day")

                open_positions[symbol] = {"side":"long","qty":qty,"entry_price":current_price}
            ...
            # SHORT ENTRY
                ...
                order = submit_smart_entry(symbol, qty, "sell", current_price)

                # TP: 3 shares
                tp_price = round(current_price * (1 - TP_PERCENT), 2)
                api.submit_order(symbol=symbol, qty=3, side="buy", type="limit", limit_price=tp_price, time_in_force="day")

                # SL: all shares
                sl_price = round(current_price * (1 + SL_PERCENT), 2)
                api.submit_order(symbol=symbol, qty=qty, side="buy", type="stop", stop_price=sl_price, time_in_force="day")

                open_positions[symbol] = {"side":"short","qty":qty,"entry_price":current_price}
